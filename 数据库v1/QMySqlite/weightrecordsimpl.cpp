...
WeightRecordsImpl::WeightRecordsImpl()
{
    ...
    //设置数据库
    database.setDatabaseName("../D28QT.db");
    //打开连接
    if( !database.open())
    {
...
int WeightRecordsImpl::isComplete(QString carId)//根据车号来查询该车辆是否有未完成的二次过磅,若有则返回记录id
{
    ...
    if(database.open())
    {
        ...
        QSqlQuery query;
        QString select_sql = "select id from WeightRecords where carId=\""+carId+"\""; //and complete=1 and ponderation=2";
    ...
    if(database.open())
    {
        ...
        else
        {
            while(query.next())
            {
                ...
                weightRecords.setPonderation(query.value(15).toInt());
                weightRecords.setComplete(query.value(16).toInt());
...
int WeightRecordsImpl::updateWeightRecords(WeightRecords weightrecords)//用来更新二次过磅的第二次过磅数据
{
    if(database.open())
    {
        ...
        QString update_sql = "update WeightRecords set date2=?,time2=?,complete=?,grossWeight=?,tare=?,netWeight=?,factoryIn=?,factoryOut=?,deduction=? where id="+id;
